...
function build_glibc()
{
	...
	_VER_P=$_GLIBC_P_VER
	_PACKAGE_P=glibc-ports-${_VER_P}.tar.bz2
	...
	prepare_package ${_PACKAGE_P} ${_GNU_MIRRORS}/glibc glibc-$step/ports

	cd $dir

	export CC="${_HOST_DIR}/usr/bin/${_TARGET}-gcc"
	export CXX="${_HOST_DIR}/usr/bin/${_TARGET}-g++"
	export CFLAGS="-O2 ${_MABI}"
	export CXXFLAGS="-O2 ${_MABI}"
